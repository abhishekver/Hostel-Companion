...
$fee_str_id = $_POST[fee_str_id];
 ...
 $resultfee=mysqli_query($dbconnection,"SELECT * FROM fees_structure where course_id='$_GET[courseid]'");
    while($rsfee=mysqli_fetch_array($resultfee))
    {
		if($rsfee[fee_type] == "Mess Advance")
		{
	$result = mysqli_query($dbconnection,"insert into mess_bill(reg_id,fee_str_id,date,mess_bill,status)values('$_GET[regid]','$rsfee[fee_str_id]','$dt','$rsfee[cost]','Enabled')");		
		}
		else
		{
	$result = mysqli_query($dbconnection,"insert into fees(reg_id,fee_str_id,total_fees,invoice_date,status)values('$_GET[regid]','$rsfee[fee_str_id]','$rsfee[cost]','$dt ','Enabled')");			
		}
	}
}
include("header.php");
...
<form method="post" action="">
 <table class="tftable" width="922" border="1">
	...
	$dt = date("d-m-Y");
    $totalfee=0;
    $result=mysqli_query($dbconnection,"SELECT * FROM fees_structure where course_id='$_GET[courseid]'");
